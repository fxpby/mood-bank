...
| Record Detail edit/delete | Done | `/record/:id` can edit one saved episode and delete one saved episode after explicit confirmation. Editing updates the selected episode fields and recomputes that episode's account impacts from current rules, so derived summaries update without direct balance editing. Deletion removes the episode and its account-impact sources from derived summaries, removes episode-linked anchors from the record delete action, preserves linked later topics / discovery points, and Topic Detail shows "来源记录已删除" instead of a broken source-record link. | No undo, version history, bulk edit/delete, or linked-topic deletion in MVP. Keep storage-failure copy honest and never delete discovery points silently. Editing must not auto-rewrite linked discovery point source snapshots. |
