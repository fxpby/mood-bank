...
| Topics List | Done | `/topics` shows discovery points, questions, topics, action ideas, local text search across saved point text/source snippets, type/status/theme/source filters, low-pressure status copy, single or 1-8 row manual creation, and inline status changes. Batch creation ignores blank rows and saves titled rows in one store action without account impact. | Keep avoiding due dates, overdue states, streaks, or unfinished-debt language. Search and batch creation must remain local/manual, not AI analysis, automatic extraction, semantic ranking, or automatic theme inference. |
| Topic Detail | Done | `/topics/:id` shows source context, detail rows, editable discovery point fields, review note, anchor save, low-pressure status actions, delete confirmation, and can turn a discovery point into a small practice with `source: "discovery_point"`. Editing/deleting only changes `AppState.topics`; source records, anchors, experiments, attempts, and derived storage-jar summaries stay unchanged. | Preserve no account impact for saving/reviewing/editing/deleting/converting by default. Do not add due dates, task debt, automatic status changes, bulk delete, undo, or linked-source deletion. |
| Record Detail edit/delete | Done | `/record/:id` can edit one saved episode, save a new episode-linked discovery point from the review surface, and delete one saved episode after explicit confirmation. Editing updates the selected episode fields and recomputes that episode's account impacts from current rules, so derived summaries update without direct balance editing. New linked discovery points preserve the record title/facts as source snapshots and create no account impact. Deletion removes the episode and its account-impact sources from derived summaries, removes episode-linked anchors from the record delete action, preserves linked later topics / discovery points, and Topic Detail shows "来源记录已删除" instead of a broken source-record link. | No undo, version history, bulk edit/delete, batch topic extraction, or linked-topic deletion in MVP. Keep storage-failure copy honest and never delete discovery points silently. Editing must not auto-rewrite linked discovery point source snapshots. |
...
| Quick Record calm closure | Done | After `/record/new` saves an episode successfully, completion copy says the record is enough for now, keeps unfinished meaning safely unfinished, can open the saved record detail, can return Home, can route to Return-to-Self with a calm-closure anchor when activation is high, can open Topics when the user chose "保存一个话题", and can continue to Seeing / Being Seen when the record contains obvious warmth. The completion actions create no extra account impacts or additional writes. | Keep this as a low-cognitive landing. Do not add a separate persisted calm-closure model, reward mechanic, or relationship conclusion. |
...
| Repair / Understanding | Done | `/repair-understanding` helps after conflict or mismatch by naming what the user wants understood, what they may not yet understand, their part to own, one next direction, and optional note. It can save one relationship-learning discovery point and creates no account impact. | Do not generate apologies, decide who was right/wrong, infer partner intent, predict repair outcome, or turn responsibility into forced self-blame. |
...
* [ ] Topics tab shows later topics, discovery points, questions to explore, and action ideas with local text search plus filters by type, status, theme, and source.
