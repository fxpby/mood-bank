...
| Record Detail delete | Done | `/record/:id` can delete one saved episode after explicit confirmation. Deletion removes the episode and its account-impact sources from derived summaries, removes episode-linked anchors from the record delete action, preserves linked later topics / discovery points, and Topic Detail shows "来源记录已删除" instead of a broken source-record link. | No undo, edit, bulk delete, or linked-topic deletion in MVP. Keep storage-failure copy honest and never delete discovery points silently. |
