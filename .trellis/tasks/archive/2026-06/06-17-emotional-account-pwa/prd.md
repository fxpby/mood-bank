...
| Topics List | Done | `/topics` shows discovery points, questions, topics, action ideas, local text search across saved point text/source snippets, type/status/theme/source filters, low-pressure status copy, manual creation, and inline status changes. | Keep avoiding due dates, overdue states, streaks, or unfinished-debt language. Search must remain local text matching, not AI analysis, semantic ranking, or automatic theme inference. |
