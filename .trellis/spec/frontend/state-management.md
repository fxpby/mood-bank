...
- Seeing evidence branch saves must use `theme: "relationship_learning"` and must not create Connection / Self / Energy impacts in the first build. It may help the user name observable being-seen or mutual-listening evidence, but must not treat warmth as future proof, infer the other person's psychology, create an empathy score, or require the user to keep listening/responding when capacity is low.
- Healthy love literacy branch saves must use `theme: "relationship_learning"` and must not create Connection / Self / Energy impacts in the first build. It may help the user distinguish care, attachment alarm, control, novelty chasing, repair willingness, boundary needs, and ordinary care in one moment, but must not become a true-love test, compatibility score, partner green/red flag score, attachment diagnosis, relationship verdict, or stay/leave advisor.
- Repair / understanding branch saves must use `theme: "relationship_learning"` and must not create Connection / Self / Energy impacts in the first build. It may help the user name what they want understood, what they may not yet understand, their part to own, and one next direction after conflict or mismatch, but must not generate apologies, decide who was right/wrong, infer partner intent, predict repair outcome, or turn responsibility into forced self-blame.
- Connection continuity branch saves must use `theme: "relationship_learning"` and must not create Connection / Self / Energy impacts in the first build. It may help the user separate collapsed felt connection from observable facts and choose one stabilizing action, but must not diagnose attachment style, create an object-constancy or connection-permanence score, monitor external signals, infer partner intent, or promise relationship permanence.
